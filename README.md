...
| `quick delete SITE` | Delete a remote site after typed confirmation or `--yes`, archiving it for restore. | `--profile <profile>`, `--yes`, `--json` |
| `quick restore SITE` | Restore a recently deleted site from the archive path printed by delete. | `--profile <profile>`, `--from <archive>`, `--yes`, `--json` |
| `quick rollback SITE` | Restore the previous or selected remote release after typed confirmation or `--yes`. | `--profile <profile>`, `--to <release>`, `--yes`, `--json` |
...
| `quick domain add\|remove\|list [domain]` | Manage custom domains in the host catalog; list includes DNS/TLS readiness. | `--site <site>`, `--profile <profile>`, `--json` |
...
Example sites live under `examples/sites/`:

- `blank/` is the smallest static site.
- `realtime-vote/` demonstrates DB and realtime channels.
- `uploads/` demonstrates upload APIs.
- `states-demo/` demonstrates accessible loading, empty, error, permission-denied, rate-limited, and offline UI states with caught SDK failures instead of unhandled console errors.

Validate state coverage with `scripts/validate-examples-states.py`; `just test-all` runs it after component tests.
